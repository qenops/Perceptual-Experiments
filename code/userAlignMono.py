...
class AlignMonoExperiment(Experiment):
    ...
    def setupStimuli(self):
        ...
        self.states = [self.horizStim, self.rightStim + [promptEnd]]
        if not self.newUser:
    ...
    def proceedure(self):
        ...
        while not self.done:
            ...
            self.wait=True
            self.clearStimuli()
        self.ipd = self.rightStim[0].start[0]
...
if __name__ == '__main__':
    config.storeData = False
    experiment = AlignMonoExperiment(config)
    experiment.run()
